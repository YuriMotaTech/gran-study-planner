...
- E2E (Playwright): `cd frontend && npm run test:e2e:install` (once). Start the stack (`docker compose up`), ensure `VITE_API_URL` matches the API (default `http://localhost:8080`). When the browser loads the app from a different origin than the API (e.g. Vite on port 5173 vs API on 8080), set `CORS_ALLOW_ORIGIN` in `.env` to that front-end origin (comma-separated list allowed) so login and API calls succeed in real browsers. Build and preview the front (`npm run build && npm run preview`), then in another terminal `cd frontend && npm run test:e2e`. CI runs the same flow against MySQL + PHP + Vite preview (see `.github/workflows/ci.yml`).
